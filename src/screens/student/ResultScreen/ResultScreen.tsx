...
export default function StudentResultScreen() {
  const { selectedStudent, token } = useAuth();
  ...
  const previewUri = resolveImageUri(
    resolveProtectedDocumentUrl(preview.previewUrl || null, token),
  );
  const pdfUri = resolveImageUri(
    resolveProtectedDocumentUrl(preview.pdfUrl || null, token),
  );
